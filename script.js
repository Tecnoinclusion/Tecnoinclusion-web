  ...
  wrap.querySelectorAll('.tf-item').forEach(item=>{
    ...
    btns.forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        if(chosen !== correct){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); }
        else { totalStars++; playCorrect(); spawnFloatingStar(btn); }
        updateStarsBadge();
      });
    });
  });
  ...
  wrap.querySelectorAll('.quiz-opts').forEach(group=>{
    ...
    btns.forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        if(parseInt(btn.dataset.o, 10) !== correctIdx){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); }
        else { totalStars++; playCorrect(); spawnFloatingStar(btn); }
        updateStarsBadge();
      });
    });
