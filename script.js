    ...
    if(doc.exists){
      ...
      if(data.activityStats){
        Object.keys(activityStats).forEach(k=>{
          if(data.activityStats[k]) Object.assign(activityStats[k], data.activityStats[k]);
        });
      }
  ...
  /* ---------- GRADO DÉCIMO ---------- */
  {
    ...
    subtemas:[
      {icon:'<i class="bi bi-123"></i>', label:'Sistemas de numeración', text:'El sistema binario usa solo dos símbolos (0 y 1) porque así "piensan" los circuitos de un computador: con corriente (1) o sin corriente (0). El sistema octal usa 8 símbolos (0 al 7) y el hexadecimal usa 16 (0 al 9, y después A, B, C, D, E, F) — estos dos existen porque son más cortos de escribir que un número binario larguísimo, y los programadores los usan para representar colores, direcciones de memoria y mucho más. Aprenderás a convertir números de un sistema a otro, entendiendo que cada sistema es simplemente "otra forma de contar lo mismo".',
        ...
        educaplay:[{url:'https://es.educaplay.com/juego/30546263-conceptos_basicos_de_informatica.html', title:'Conceptos básicos de informática'}]},
      {icon:'<i class="bi bi-lightning-charge-fill"></i>', label:'Electricidad y electrónica', text:'Antes de armar o reparar cualquier equipo, necesitas entender la electricidad que lo hace funcionar. La carga eléctrica es la propiedad de la materia que produce fuerzas eléctricas; la resistencia es lo que se opone al paso de la corriente; la tensión (o voltaje) es la fuerza que "empuja" los electrones; la corriente es el flujo de esos electrones; y la potencia y la energía indican cuánto trabajo puede hacer esa electricidad. Estos cinco conceptos son la base de toda la electrónica que verás en los próximos periodos.',
        ...
        educaplay:[{url:'https://es.educaplay.com/juego/30546308-electricidad_en_orden.html', title:'Electricidad en orden'}]},
  ...
  },
  {
    ...
    subtemas:[
      ...
      {icon:'<i class="bi bi-laptop"></i>', label:'Programación en C++ y Python', text:'C++ y Python son dos lenguajes de programación muy usados: C++ es más cercano a cómo funciona la máquina por dentro (por eso se usa mucho en sistemas que necesitan ser muy rápidos), mientras que Python tiene una sintaxis más sencilla de leer, ideal para aprender lógica de programación. En ambos aprenderás sobre variables (donde guardas un dato que puede cambiar, como la edad de una persona), constantes (un valor que nunca cambia, como el número Pi) y ciclos repetitivos (estructuras como "for" o "while" que repiten una acción muchas veces sin que tengas que escribirla una y otra vez).',
        ...
        educaplay:[{url:'https://es.educaplay.com/juego/30545986-c_completar_palabras.html', title:'C++: completar palabras'}]},
  ...
  /* ---------- GRADO ONCE ---------- */
  {
    ...
    subtemas:[
      {icon:'<i class="bi bi-plug-fill"></i>', label:'Tarjeta Arduino', text:'Arduino Uno es una placa con un microcontrolador (un pequeño "cerebro" programable), pines digitales y analógicos para conectar sensores y actuadores, un puerto USB para programarla desde el computador, y un regulador de voltaje que asegura que todo funcione con la energía correcta. Su software (el IDE de Arduino) te permite escribir el código, llamado "sketch", y cargarlo directamente en la placa.',
        ...
        educaplay:[{url:'https://es.educaplay.com/juego/30504773-quiz_arduino_facil.html', title:'Quiz Arduino Fácil'}]},
  ...
  },
  {
    ...
    subtemas:[
      ...
      {icon:'<i class="bi bi-cloud-fill"></i>', label:'Computación en la nube', text:'La computación en la nube permite usar programas y almacenar información a través de internet, sin necesitar instalarlo todo en tu propio computador. SaaS (Software as a Service) es un programa que usas directamente desde el navegador, como el correo electrónico. PaaS (Platform as a Service) ofrece una plataforma lista para que los desarrolladores construyan aplicaciones sin preocuparse por la infraestructura. IaaS (Infrastructure as a Service) alquila la infraestructura misma (servidores, almacenamiento) para que una empresa monte lo que necesite sobre ella.',
        ...
        mq:[{q:'El correo electrónico que usas desde el navegador es un ejemplo de...', opts:['IaaS','SaaS','Ninguno','PaaS'], correct:1},{q:'¿Qué permite usar programas sin instalarlos en tu computador?', opts:['Un disco duro externo','Un antivirus','La computación en la nube','Nada'], correct:2}], pic:[{emoji:'☁️', q:'Esta imagen representa...', opts:['Un dibujo','La computación en la nube','El clima','Un videojuego'], correct:1},{emoji:'📧', q:'Usar el correo electrónico desde el navegador es un ejemplo de...', opts:['Ninguno','PaaS','IaaS','SaaS'], correct:3}], imgloc:[{q:'¿Cuál de estas imágenes representa la computación en la nube?', pics:['❄️','🌞','☁️','🌧️'], correct:2},{q:'¿Cuál de estos íconos representa un servicio que usas sin instalarlo, como el correo?', pics:['💽','🖥️','📧','🖨️'], correct:2}]}
    ],
...
/* Estadísticas independientes por tipo de actividad (aciertos/intentos), para que cada
   actividad tenga su propia nota de 10 a 100 sin mezclarse con las demás. */
const activityStats = {
  fc: { correct:0, total:0 },
  tf: { correct:0, total:0 },
  mq: { correct:0, total:0 },
  pic: { correct:0, total:0 },
  imgloc: { correct:0, total:0 }
};

/* Convierte un aciertos/total en una nota de 10 a 100. Si no hay intentos, devuelve null (sin nota aún). */
function gradeFromRatio(correct, total){
  if(!total || total <= 0) return null;
  return Math.min(100, Math.max(10, Math.round(10 + 90 * (correct/total))));
}

function temasDeGrado(g){ return temas.filter(t => t.grado === g); }

/* Calcula el máximo real de estrellas que se pueden ganar en todo el contenido de un grado,
   contando exactamente cada actividad (para que el puntaje del profesor sí cuadre con las estrellas). */
function maxPossibleStars(grado){
  let max = 0;
  temasDeGrado(grado).forEach(t=>{
    if(t.game && t.game.word) max += t.game.word.length + 3; // sopa de letras: 1 por letra + 3 de bono
    if(t.dragline) max += t.dragline.length + 3; // arrastra las líneas: 1 por pareja + 3 de bono
    if(t.quiz) max += t.quiz.length + 3; // evaluación final: 1 por pregunta correcta + 3 de bono
    (t.subtemas || []).forEach(s=>{
      max += (s.fc||[]).length + (s.tf||[]).length + (s.mq||[]).length + (s.pic||[]).length + (s.imgloc||[]).length;
    });
  });
  return max;
}
...
/* Pequeño efecto que sale del botón al acertar: más ágil que el confeti completo,
   pensado para dar vida a cada actividad chica sin saturar la pantalla. */
function spawnFloatingStar(el){
  if(!el) return;
...
/* Explosión de confeti pequeña justo donde el estudiante tocó, para que cada acierto
   se sienta como una mini-celebración (no solo un cambio de color). */
function miniConfetti(x, y){
  ...
  for(let i=0;i<14;i++){
    ...
    piece.style.setProperty('--mx', (Math.cos(angle)*dist) + 'px');
    piece.style.setProperty('--my', (Math.sin(angle)*dist) + 'px');
...
/* Sacudida breve cuando la respuesta es incorrecta, para dar feedback físico e inmediato. */
function shakeEl(el){
  if(!el) return;
  el.classList.remove('shake-wrong');
  void el.offsetWidth; // reinicia la animación si se hace clic varias veces seguidas
...
function subtemaBlockHtml(t, s, idx){
  ...
  const educaplayHtml = (s.educaplay || []).map(e => `
    <div class="activity-card educaplay-card">
      <div class="activity-head educaplay-head"><i class="bi bi-joystick"></i> ${e.title || 'Juego de Educaplay'}</div>
      <p class="activity-hint">Actividad externa, se juega aquí mismo.</p>
      <div class="educaplay-frame-wrap">
        <iframe src="${e.url}" allow="fullscreen; autoplay; allow-top-navigation-by-user-activation" allowfullscreen loading="lazy"></iframe>
      </div>
    </div>`).join('');
  ...
  wrap.querySelectorAll('.flip-card').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      if(first && btn.classList.contains('flipped')){
        ...
        totalStars++;
        activityStats.fc.total++;
        activityStats.fc.correct++;
        updateStarsBadge();
        spawnFloatingStar(btn);
      }
      playCorrect();
    });
  });
  ...
  wrap.querySelectorAll('.tf-item').forEach(item=>{
    ...
    btns.forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        if(chosen !== correct){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); activityStats.tf.total++; }
        else { totalStars++; playCorrect(); spawnFloatingStar(btn); activityStats.tf.total++; activityStats.tf.correct++; }
        updateStarsBadge();
      });
    });
  });
  ...
  wrap.querySelectorAll('.quiz-opts').forEach(group=>{
    ...
    btns.forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        if(parseInt(btn.dataset.o, 10) !== correctIdx){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); activityStats.mq.total++; }
        else { totalStars++; playCorrect(); spawnFloatingStar(btn); activityStats.mq.total++; activityStats.mq.correct++; }
        updateStarsBadge();
      });
    });
  });
  ...
  wrap.querySelectorAll('.quiz-opts').forEach(group=>{
    ...
    btns.forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        btns.forEach(b=> b.disabled = true);
        btns[correctIdx].classList.add('correct');
        if(parseInt(btn.dataset.o, 10) !== correctIdx){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); activityStats.pic.total++; }
        else { totalStars++; playCorrect(); spawnFloatingStar(btn); activityStats.pic.total++; activityStats.pic.correct++; }
        updateStarsBadge();
      });
    });
  });
  ...
  wrap.querySelectorAll('.imgloc-grid').forEach(group=>{
    ...
    btns.forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        btns.forEach(b=> b.disabled = true);
        btns[correctIdx].classList.add('correct');
        if(parseInt(btn.dataset.o, 10) !== correctIdx){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); activityStats.imgloc.total++; }
        else { totalStars++; playCorrect(); spawnFloatingStar(btn); activityStats.imgloc.total++; activityStats.imgloc.correct++; }
        updateStarsBadge();
      });
    });
  });
...
function setupQuiz(t){
  ...
  document.querySelectorAll('#evalPanel .quiz-opt').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      if(oi !== correctIdx){ btn.classList.add('wrong'); shakeEl(btn); playWrong(); } else { correctCount++; totalStars++; playCorrect(); spawnFloatingStar(btn); updateStarsBadge(); }
      answered++;
      if(answered === t.quiz.length){
        ...
        playFanfare();
        confettiBurst();
        feedback.textContent = `Terminaste. Acertaste ${correctCount} de ${t.quiz.length}. +3 estrellas de bono. Insignia ganada.`;
        updateEvalDashboard();
        renderEvalTabs();
        saveProgressToCloud();
      }
